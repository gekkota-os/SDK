...
JustGage = function(config) {
  ...
  obj.config = {
    ...
    // formatNumberSymbol : string
    // the format symbol to use (comma by default)
    formatNumberSymbol : kvLookup('formatNumberSymbol', config, dataset, ','),
  ...
  if (obj.config.maxTxt) {
    ...
    obj.txtMaximum = formatNumber(max, obj.config.formatNumberSymbol);
  }
  ...
  if (obj.config.textRenderer) {
    ...
    obj.originalValue = formatNumber(obj.originalValue, obj.config.formatNumberSymbol) + obj.config.symbol;
  } else {
  ...
  if (obj.config.counter === true) {
    ...
    eve.on("raphael.anim.frame." + (obj.level.id), function() {
      ...
      if (obj.config.textRenderer) {
        ...
        obj.txtValue.attr("text", formatNumber(Math.floor(currentValue), obj.config.formatNumberSymbol) + obj.config.symbol);
      } else {
...
JustGage.prototype.refresh = function(val, max, min) {
  ...
  if (max !== null) {
    ...
    if (obj.config.maxTxt) {
      ...
      obj.txtMaximum = formatNumber(obj.config.max, obj.config.formatNumberSymbol);
    }
    if (!obj.config.reverse) {
      obj.txtMax.attr({
        "text": obj.txtMaximum
  ...
  if(min !== null){
    ...
    if( obj.config.humanFriendly ) {
      ...
      obj.txtMinimum = formatNumber(obj.config.min, obj.config.formatNumberSymbol);
    }
  ...
  if (obj.config.textRenderer) {
    ...
    displayVal = formatNumber((displayVal * 1).toFixed(obj.config.decimals), obj.config.formatNumberSymbol) + obj.config.symbol;
  } else {
...
function formatNumber(x, symb) {
  //console.log('formatNumber('+x+', '+symb+')');
  if ('undefined' === typeof symb) {
    symb = ",";
  }
